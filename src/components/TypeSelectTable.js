      ...
      <tbody>
        {
          props.questions.map((val, idx) => (
            <tr key={idx} className={val.type === Types.DEMOGRAPHIC ? "table-danger" : val.type === Types.QUALITATIVE ? "table-info" : ""}>
              ...
              <td>
                {/* <input type="checkbox" className="form-control" value={check[idx]} onChange={(event) => {console.log(event); let a = check; a[idx] = !a[idx]; setCheck(a);}}></input> */}
                <input type="checkbox" checked={val.hasLink} disabled={val.type !== Types.QUALITATIVE} onChange={(event) => {props.updateQuestion(idx, val.type, !val.hasLink); console.log(event)}}></input>
              </td>
              <td>
                <input type="number" className="form-control" placeholder="N/A" value={val.link} disabled={val.type !== Types.QUALITATIVE} onChange={(event) => props.updateQuestion(idx, val.type, true, event.target.value)}></input>
              </td>
            </tr>
          ))
        }
